...
// ── 왕복 자동 확정 지속시간 (v2.18 → v2.x 개정)
// 왕복(모터 상세·레이스 진입) 자동 확정의 **연속 측정 시간 하한**. 누적이 아니라 연속이다 —
// 신호가 끊기면 0부터 다시 센다(끊긴 구간을 합치면 하한의 의미가 없다).
//
// v2.x(사용자): 자동 확정 트리거를 엔진 stable 판정(1.5초 CV<1.5%) 의존에서 분리한다.
// stable에 의존하면 CV가 계속 1.5% 이상인 "흔들림 큼" 모터는 영영 안정에 도달하지 못해
// 왕복 경로로 **아예 기록되지 않는 사각지대**가 생긴다(컨디션 추적 대상이 정작 안 잡힘).
// 그래서 확정 조건을 "안정 여부 무관 + 연속 8초 + CV 계산됨(창 참)"으로 바꾼다 — 불안정
// 모터도 높은 CV 그대로 기록된다. 좋은 모터는 8초면 이미 안정이라 median 잠금값이 기록된다.
// 하한을 5초→8초로 늘려(사용자) 초기 회전 안정화에 더 여유를 준다.
// standalone [기록](측정 탭 직접)은 이 하한 미적용 — measuring이면 즉시 기록(v2.23).
export const MIN_MEASURE_DURATION_MS = 8000
