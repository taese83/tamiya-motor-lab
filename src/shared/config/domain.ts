...
// ── 왕복 자동 확정 지속시간 (v2.18 → v2.x: 3s+5s → R60: 5s 안정-우선 + 10s 타임아웃)
// 왕복(모터 상세·레이스 진입) 자동 확정의 **연속 측정 시간**. 누적이 아니라 연속이다 —
// 신호가 끊기면 0부터 다시 센다(끊긴 구간을 합치면 의미가 없다).
//
// R60(사용자: 튜너 모드로 검출이 안정화됐으니 5초 이후 안정 시점에 측정):
//  ① MIN(5초): 초기 5초(스핀업 과도 + 안착)는 안정 판정을 **무시**하고, 5초 이후
//     **안정(isStable)**이 뜨는 순간 확정한다 — 확정값은 충분히 안착한 1.5s 창 중앙값.
//  ② MAX(10초): 그래도 안정에 도달하지 못하면 그 시점 마지막 측정값으로 확정한다. stable에만
...
export const MAX_MEASURE_DURATION_MS = 10000
