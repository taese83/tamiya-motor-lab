...
export interface EngineTuning {
  ...
  silenceRms: number
  /**
   * 신뢰 경계 RMS (R68에서 차단선 → 경계로 강등, 파노튜너 모델) — 이 값 이상이면 종전과
   * 동일하게 동작하고, 미만인 조용한 프레임도 분석은 하되 **엄격한 스펙트럼 라인 증거**
   * (gateSnrDb·gateMinHarmonics)를 통과해야만 측정로 인정한다. 원거리 확산 소음은 라인이
   * 없어 기각되고(weak-signal 'too-quiet' 안내 유지), 원래 소리가 작은 모터(토크튠·렙튠·
   * 130)는 라인 실증거로 통과한다. 근접 방어의 본체는 음량 차단이 아니라 거리 감쇠 물리
   * (10cm vs 2m ≈ 26dB) + 우세 소리원 선택이다.
