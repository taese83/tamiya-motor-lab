...
export function MeasurePage() {
  ...
  useRaceAutoCollect({
    // v2.x(사용자): 안정-우선 + 타임아웃. measuredMs는 **모터 소리 입력 순간부터의 연속 측정
    // 시간**이다(화면 진입 시각 아님 — 무신호면 0으로 리셋). ① 3초 이후 안정(isStable)이면 즉시
    // 확정(초기 3초 스핀업 과도는 안정 무시, 좋은 모터는 5초를 다 안 기다림). ② 5초까지 안정 못
    // 하면 마지막 값으로 확정 — "흔들림 큼"(CV≥1.5%) 모터가 왕복 경로로 아예 기록 안 되던
    // 사각지대를 타임아웃이 막는다(사용자 지적).
    readyToCollect:
      view.status === 'measuring' &&
      ((view.isStable && view.measuredMs >= MIN_MEASURE_DURATION_MS) ||
        view.measuredMs >= MAX_MEASURE_DURATION_MS),
    ...
    <Box
      ...
      */}
      ...
      <Box sx={{px: 2, flex: 1, display: 'flex', flexDirection: 'column'}}>
        ...
        {/* [Z2c] 신호 세기 미터 (R45→R46 수신감도 바→R48 사용자: 안정도 쪽으로 조금 위로) —
            위 스페이서(1) < 아래 스페이서(2)로 하단 영역의 위쪽(≈안정도에 더 가깝게) 배치.
            "신호 약함"·"더 가까이"를 하나로 통일. 상태 전환에도 위치 불변. */}
        <Box>
