...
export function AuthMenu() {
  ...
  // 비로그인 = 바로 로그인 진입 / 로그인 = 계정 메뉴 오픈
  const handleAvatarClick = (e: MouseEvent<HTMLElement>): void => {
    if (user === null) {
  ...
  return (
    <>
      <IconButton
        ...
        aria-label={user !== null ? `${user.name} 계정 메뉴` : '구글 로그인'}
        {...(user !== null ? {'aria-haspopup': 'menu' as const, 'aria-expanded': open} : {})}
        sx={{p: 0.5}}>
        {/* src 없으면 MUI Avatar가 기본 사람 실루엣을 렌더 — 비로그인/사진없음 공통 */}
        <Avatar {...(user?.picture ? {src: user.picture} : {})} alt="" sx={{width: 32, height: 32}} />
      </IconButton>

      {/* 로그인 상태에서만 계정 메뉴(기본 정보 + 로그아웃). 비로그인은 아바타 클릭이 바로 로그인 진입 */}
      {user !== null && (
        <Menu
          anchorEl={anchorEl}
          open={open}
