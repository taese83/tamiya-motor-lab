// S1 스크린리더 알림 문구 계약 (component-spec §2.6) — 순수 함수 계층 (unit 테스트 대상).
// 알림 채널은 MeasureStatusLabel 내부 hidden role="status" 1곳뿐이며, 이 모듈은
// "상태 전이 도착 시점의 문구"와 "debounce·중복 억제 판정"만 계산한다.
// 수치 갱신(≥10 Hz)은 절대 announce하지 않는다 — stable 문구의 수치 포함은 전이 1회 알림.
...
/** 전이 도착 상태 → 알림 문구 (component-spec §2.6 표 고정 copy) */
export function buildAnnouncement(view: MeasureView): string {
  switch (view.status) {
    ...
    case 'stable':
      return `측정 완료, ${formatRpm(view.rpm)} RPM`
    case 'no-permission':
