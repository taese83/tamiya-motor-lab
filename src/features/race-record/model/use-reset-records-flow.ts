...
import {useResetMotorRaceRecords} from '../api/mutations'

// v2.3 모터별 [레이스 기록 초기화] 플로우 어댑터 (레이스 상세 하단) — ResetRecordsBlock의 `onReset: () => Promise<boolean>` 계약.
// confirm·pending·오류 표시는 ResetRecordsBlock 내부 상태 머신 소유 — 이 훅은 실행·토스트만.
// invalidation(raceKeys.byMotor·motorKeys.summaries — measure/motors 캐시 유지)은
// useResetMotorRaceRecords 소관.
...
export function useResetRecordsFlow(motorId: string): ResetRecordsFlow {
  ...
  const mutation = useResetMotorRaceRecords()
