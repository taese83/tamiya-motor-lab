...
export interface ResetRecordsBlockProps {
  /** 초기화 대상 모터 이름 — confirm 범위 고지에 사용 (v2.3: 레이스 기록만 초기화) */
  motorName: string
  /**
...
/**
 * 레이스 상세(`/race/:motorId`) 최하단 [레이스 기록 초기화] — v2.3 사용자 결정 정정.
 * 해당 모터의 **레이스 기록만** 삭제, 측정(파노) 기록·모터 등록·다른 모터 기록은 유지.
...
export function ResetRecordsBlock({motorName, onReset}: ResetRecordsBlockProps) {
  ...
  return (
    <Box>
      ...
      <ConfirmDialog
        open={state.open}
        title={`'${motorName}'의 레이스 기록을 초기화할까요?`}
        impact={`'${motorName}'의 레이스 기록이 모두 삭제됩니다. 측정(파노) 기록과 모터 등록, 다른 모터의 기록은 유지됩니다. 되돌릴 수 없습니다.`}
        confirmLabel="초기화"
