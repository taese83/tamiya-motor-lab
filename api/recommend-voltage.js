...
export default async function handler(req, res) {
  ...
  const system = [
    ...
    '- **파노는 레이스 전에 이미 측정된 모터 상태**다(내가 만들 결과가 아니다). 파노가 올랐다는 것은 같은 전압에서 이미 더 빨라졌다는 뜻이므로, 같은 주행 속도를 유지하려면 전압을 **낮춰야** 한다. 파노가 내렸으면 반대로 올린다.',
    '- 판단 기준은 속도 지표 **S = 파노 × 전압**이다. 과거 완주 기록에서 S(성공했던 속도 수준)를 잡고, 현재 파노 P에서 전압 = S / P로 역산한다. 완주 기록이 없으면 전체 기록으로 대신하되 확신을 낮춘다.',
    ...
    '1) 완주(finished) 기록의 속도 지표 S=파노×전압을 weight로 가중 평균해 목표 속도 S̄를 잡는다(완주 기록이 없으면 전체 기록으로 대신하되 확신을 낮춘다). 최근(weight 큰) 기록 우선.',
