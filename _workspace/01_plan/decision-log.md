...
### DL-031 — 전압 규칙 재조정: 인용 허용·처방 금지, 위반은 섹션 드롭 (2026-08-01, 사용자)
- **배경(실사용 실패)**: R25/R26 배포 후 AI 분석이 항상 실패. Vercel 로그 `code: voltage_pattern_rejected` — 모델은 7초에 정상 응답했으나 **서버의 전압 패턴 검사가 응답 전체를 502로 거부**.
- **근본 원인(오케스트레이터 설계 오류)**: DL-029의 결정은 "**코칭 섹션이 전압을 추천하지 말 것**"이었는데, 구현에서 "모든 섹션 전압 수치 전면 금지 + 서버 하드 거부"로 **과잉 적용**했다. 처방(prescribe)과 인용(cite)을 구분하지 않은 것이 핵심 오류. 게다가 주입 데이터(insight)에 완주 전압대·최근 완주 전압이 포함되고 프롬프트가 "주입값을 인용하라"고 지시하므로, 모델이 전압을 인용하는 것은 **지시를 따른 결과**였다 — 구조적 자기모순.
- **DL-031 확정**:
  ① **인용 허용·처방 금지** — diagnosis·anomaly·briefing은 주입된 전압값 인용 가능. `nextRace`(다음 판 제안) 섹션에서만 전압 수치 금지(방향 어휘만).
  ② **위반은 섹션 드롭** — `nextRace`에 전압 수치가 있으면 그 섹션만 제거하고 나머지 분석은 정상 반환(200). 502 전체 실패 폐기. 단 드롭 후 섹션이 0개면 기존 규칙대로 502(빈 응답 금지).
- 근거: 문체 규칙 위반으로 전체 분석을 날리는 것은 과잉이며, 안전 목적(과전압 유도 방지)은 nextRace 한정 검사로 충분히 달성된다. DL-029의 원 의도(추천은 기존 advisor 소관)는 그대로 유지.
- 영향: api/analyze-race.js(VOLTAGE_PATTERN 적용 범위·프롬프트 문구), eval-plan G-VOLT(전 시나리오 → nextRace 한정으로 조정 필요).

### DL-032 — 분석 가중치: 스케일 완화 + 결과 기반 초점 규칙 (2026-08-01, 사용자)
- **발견**: `RACE_WEIGHT_GROWTH=1.5`는 전압 추천의 짧은 윈도우(최근 완주까지·폴백 5건, 최대 5:1)용인데, R25가 이를 분석용 20건 payload에 그대로 재사용 → **최신 2216.84 : 최고참 1 = 2217:1**. "회차 간 패턴 탐지"가 이 기능의 핵심인데 오래된 기록을 사실상 무의미하게 만들어 **기능의 근거를 스스로 지우는** 상태였다(프롬프트의 "weight 큰 기록을 더 신뢰하라"가 증폭). 원인: 윈도우가 4배 커진 걸 재검토하지 않고 상수 재사용.
- **사용자 기준(핵심)**: 가중은 "얼마나 최근인가"가 아니라 **"무엇이 정보를 주는가"**여야 한다 —
  ① 이탈하다 **완주했다면 그 완주에 집중**(무엇이 달라서 성공했는지가 답) ② **완주가 전혀 없으면 이탈에 집중**(반복 사유의 공통 원인이 답).
- **확정**:
  ① 분석 전용 상수 `ANALYZE_WEIGHT_GROWTH = 1.1` 신설 — 20건에서 약 6:1(최신 우선 신호는 유지하되 과거 기록도 근거로 생존). `RACE_WEIGHT_GROWTH`(추천용)는 **불변** — 바꾸면 추천 품질이 바뀐다.
  ② 프롬프트에 **결과 기반 초점 규칙** 추가: 완주 존재 시 가장 최근 완주를 성공 기준점으로 삼아 전후 이탈과 비교(전압·파노·사유), 완주 부재 시 반복 이탈 사유의 공통 원인에 집중.
- 근거: LLM은 수치 가중을 실제로 계산하지 않고 인상으로 받는다 — 극단적 비율은 해롭고, 초점은 **명시 지시**가 훨씬 신뢰도가 높다. 가중치는 완만한 최신성 신호로만 남기고 의미 판단은 규칙이 담당한다.

## 2026-08-01 레이스 자동 입력 기획

### DL-033 — autofill intake 확정 3건 (2026-08-01, 사용자 — 재질문 금지)

- **결정**: ① 자동화 범위 = **폼 프리필만** — [+ 기록] 열면 값이 이미 채워져 있고 사용자가 확인·수정 후 저장. 자동 저장 없음 → **L1 유지**(autonomy-risk-matrix의 L2 승격 조건 미적용 — write는 여전히 사용자 행동) ② 채울 항목 3종: 전압·목표(완주/안정/속도)·**주행 전 체크리스트**(신규 개념 — RaceRecord 필드가 아님) ③ 품질 검증(AC-5/6) 없이 진행 — 사용자 결정. 완화 요인: 프리필이라 잘못된 값이 저장 전에 보이고 수정 가능.
- **검증 방법**: race-autofill planning-context의 Critical State Inventory·Open Decisions가 이 확정과 정합한지 plan review에서 확인.

### DL-034 — 전압 프리필 출처 불변: 이번 과제의 신규 몫은 목표·체크리스트 (2026-08-01, planning-facilitator 코드 확인)

- **확인 사실**: AI 분석(analyze-race)은 전압 수치를 출력하지 않는다(DL-029/031 — nextRace 수치 금지, 방향 어휘만). 전압 프리필은 이미 `openWithGoal` 휴리스틱 + [AI 추천] LLM(use-race-entry.ts)이 담당 중이다.
- **결정(기록)**: 전압 프리필은 기존 advisor 현행 유지 — 이번 과제에서 재발명하지 않는다(DL-022 계열). "AI 추천 기반 자동 입력"의 신규 몫은 **목표 추천 표시**와 **체크리스트 파생**이며, 두 항목의 출처(결정론 파생 vs [+ 기록]마다 LLM 자동 호출)는 D1로 사용자 확인. 권고는 결정론 — analyze-race는 rate limit 5회/분·ALLOWED_EMAIL fail-closed·수 초 지연이라 프리필 경로 자동 호출에 부적합하고, 결정론은 즉시·오프라인·무료다.
- **검증 방법**: D1 사용자 결정 후 requirements에서 항목별 출처 표(전압=advisor, 목표·체크리스트=?)를 고정.

### DL-035 — 산출물 경로: race-autofill/ 하위는 오케스트레이터 대필 (DL-025 동일 계열)

- **결정**: `_workspace/01_plan/race-autofill/planning-context.md`는 ownership hook이 하위 디렉터리 쓰기를 차단하므로 facilitator가 본문을 반환하고 오케스트레이터가 저장한다. decision-log append만 직접 수행. 전체 앱용 `_workspace/01_plan/planning-context.md`는 덮어쓰지 않는다.

### DL-036~038 — race-autofill 구조 결정 확정 (2026-08-01, 사용자)
- **DL-036 (D1 프리필 근거)**: **결정론 파생** 확정 — 기록에서 규칙으로 계산(즉시·무료·오프라인). [+ 기록]마다 LLM 호출은 기각(rate limit 5/분·fail-closed·수 초 지연·오프라인 불가). 전압은 기존 advisor 현행 유지(A1 확정).
- **DL-037 (D2 목표)**: **팝업 유지 + 추천 표시** 확정 — 3택 그대로 두고 추천 목표에 배지+근거 한 줄. 선택은 사용자 몫(의사 표현 지점 보존). 팝업 스킵 기각.
- **DL-038 (D3 체크리스트)**: **비저장 표시 전용** 확정 — 체크 상태는 ephemeral(시트 닫으면 소멸), RaceRecord·스키마·서버 동기화 **무변경**. 저장안(신규 엔티티+migration, L) 기각.
- 확정 효과: 규모 **S~M**, production integration delta 0, 신규 필드·migration 0. 잔여 미결 D4(추천 목표 vs 직전 목표 강조 충돌)·D5(체크리스트 근거 없을 때 비노출 여부)는 UX/설계 단계에서 처리.

### DL-039 — race-autofill 잔여 미결 확정 (2026-08-02, 사용자 "진행해")
- **D4 확정**: 목표 팝업에서 **직전 목표 contained 강조는 현행 유지**, 추천은 **별도 배지 "추천"+근거 캡션 병기**. 동일 목표면 두 표기 공존. 자동 선택 없음(REQ-AF-003안 채택).
- **D5 확정**: 유효 이탈 사유 0건이면 체크리스트 **블록 비노출**(침묵). 고정 일반 항목 표시안 기각 — 매번 동일 항목은 습관화로 무시되고 수직 예산만 소모(REQ-AF-006안 채택).
- plan-review 경미 지적 반영 대상: ① R2의 "최신 이탈 회차"는 races[0]이 아니라 **result 확정 첫 회차** 기준(미정 회차가 더 최신일 수 있음) — 구현 계약 주석에 명시 ② N04 a11y(checkbox role·44px·SR 근거) 테스트 케이스 추가.
- 리뷰 지적 3(R2×jump_attitude 카피): 규칙 유지 + 근거 카피를 **"직전 이탈 — 안정 권장"**으로 중립화(속도 단정 회피).

### DL-040 — 전압 추천 모델 반전: 속도 유지(파노↑ → 전압↓) (2026-08-02, 사용자)
- **사용자 관찰**: 파노가 올랐는데 '안정' 목표에서 추천 전압이 **더 올라감**. 재현 확인(1건 이력: 파노 300→320에서 3.00V→3.20V / 3건 이력: 2.90V→3.10V).
- **원인(개념 오류)**: 휴리스틱이 `V ≈ a·P + b`를 학습해 현재 파노에서 평가한다. 1건이면 순수 비례(`V=(V₁/P₁)·P`, 주석 "원점 통과 물리 근사"). **파노↔전압의 인과 방향이 뒤집혀 있다** — 전압을 올리면 파노가 오르는 건 맞지만, 여기서 P는 "만들 결과"가 아니라 **레이스 전 이미 측정된 모터 상태**다. 모터가 좋아져 파노가 올랐다면 같은 전압에서 이미 더 빠른 상태이므로 전압을 올리면 과속이 가중된다. 게다가 `GOAL_DELTA.stability = 0`이라 안정 목표에는 상쇄 장치가 전혀 없다(유일한 하방 압력인 이탈 회피 cap은 유사 파노 이탈 기록이 있어야만 작동).
- **사용자 판정**: ① 파노가 오르면 전압은 **낮춰야** 한다(속도 유지) ② **세 목표 모두** 적용 — 파노 상승은 이미 빨라졌다는 뜻이므로 전 목표에서 상쇄하고, 목표 차이는 보정값(GOAL_DELTA)으로만 표현.
- **확정 모델**: **속도 유지(speed-holding)** — 속도 지표 `S = panoHz × voltage`를 이력에서 학습(완주 기록 우선·지수 가중)하고, 현재 파노 P에서 `V = S_target / P`로 역산한다. 파노 동일 → 직전 전압 그대로, 파노 상승 → 전압 하락. 목표 보정(finish -0.3 / stability 0 / speed +0.3)과 이탈 회피 cap, 2.6~3.2 클램프는 **현행 유지**.
- 표본 선택: 완주(finished) 기록이 있으면 그것만으로 S 학습(이탈 기록의 S는 과속 영역이라 상향 편향). 완주 0건이면 전체 기록으로 폴백하고 근거 문구에 표기.
- 영향: `src/shared/lib/voltage-advisor/voltage-advisor.ts`(모델) · 동 테스트(기존 "파노가 오르면 전압도 오른다" 케이스 **의미 반전**) · `api/recommend-voltage.js` 프롬프트(같은 전제를 담고 있어 함께 수정하지 않으면 [AI 추천]과 휴리스틱이 반대 방향을 말함).
