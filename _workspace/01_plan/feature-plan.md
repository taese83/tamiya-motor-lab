...
## 부록 — FEAT ID 매핑 (델타 킷 온보딩용 정규화, 2026-08-19)

> 본 계획의 기능 표는 FEAT-ID 규율 이전 형식(F1~F10)이다. 라이브 델타 traceability의
> 양방향 대조(validate-design-preview)가 리터럴 FEAT ID를 요구하므로, 기존 내용 무수정의
> 부가 절로 정식 ID를 부여한다. 이후 신규 기능은 다음 번호(011~)부터 이 절에 등재한다.

| FEAT ID | 원 표기 | Feature |
|---|---|---|
| FEAT-001 | F1 | 분석 엔진 파이프라인 v2 |
| FEAT-002 | F2 | 측정 세션 |
| FEAT-003 | F3 | 측정→기록 연결 |
| FEAT-004 | F4 | IndexedDB persistence 계층 |
| FEAT-005 | F5 | 모터 CRUD |
| FEAT-006 | F6 | 기록 CRUD |
| FEAT-007 | F7 | 목록·이력 조회 |
| FEAT-008 | F8 | 전압 가이드 계산·표시 |
| FEAT-009 | F9 | 앱 셸·탭 네비게이션 |
| FEAT-010 | F10 | 공용 UI 킷 |

as-is 온보딩 앵커의 대표 TC (기존 동작 서술 — 회귀 보증은 기존 스위트):

- TC-002-1: 측정 탭에서 마이크 권한 승인·구동음 입력 시 게이지·RPM이 실시간 갱신되고 stable 시 기록 가능하다
- TC-007-1: 모터 탭에서 모터 선택 시 상세(종류 칩·파노 차트·기록 리스트·측정 CTA)가 표시된다
- TC-008-1: 레이스 탭에서 기록 시트 진입 시 결과 세그먼트·전압 스테퍼·이탈 사유 입력이 동작한다
