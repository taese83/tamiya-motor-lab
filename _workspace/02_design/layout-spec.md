...
| **LD-4** | ~~**RaceRecord 개별 삭제 = 행 우측 [삭제] 버튼 + ConfirmDialog**~~ (RV-A3 확정, 스와이프 기각) — **v2.16에서 번복됨, 아래 행 참조** | 스와이프는 발견성이 낮고 세로 스크롤과 제스처 경합. 가시 버튼 44×44가 오입력 복구 수단으로 확실 |
| **LD-4′ (v2.16 — LD-4 번복)** | **모터 행·RaceRecord 행의 [수정]·[삭제] = 왼쪽 스와이프 트레이**. 사용자 지시로 위 기각 결정을 뒤집는다 | 기각 사유 3개를 각각 막은 뒤 도입했다. ① *발견성* → 트레이 액션은 **항상 DOM에 존재하는 진짜 button**이고 Tab 포커스가 들어오면 트레이가 자동으로 열린다(보이지 않는 컨트롤에 포커스가 앉지 않게 — WCAG 2.4.11). 아이콘 단독이 아니라 **아이콘 + 텍스트 라벨**(색·형태 단독 구분 금지 승계). ② *세로 스크롤 경합* → `touch-action: pan-y`로 세로 패닝은 브라우저에 남기고, 첫 이동에서 **방향 락**(가로 우세 + 임계 10px 초과일 때만 점유·세로 우세면 즉시 포기). DnD 핸들은 `data-swipe-ignore`로 제외해 정렬 제스처와 분리. ③ *오입력 복구* → 파괴 액션은 여전히 **ConfirmDialog 경유**이고 풀 스와이프 즉시 삭제 같은 지름길은 만들지 않았다(제스처의 최대 권한 = 트레이 열기). 모터 삭제는 cascade라 실측 건수 고지(CP-3)를 그대로 통과한다. 단일 열림은 목록이 소유(`useSingleOpenRow`) — 두 행이 동시에 열려 액션 대상이 모호해지는 상태를 구조적으로 제거 |
